...
# pretrain된 yolov8 small 모델을 model 변수에 정의
...
class VideoCamera(object):
    ...
    def get_frame(self): # 객체탐지 결과 프레임을 반환하는 함수
        ...
        # coordinates
        # model에 1장을 주면 results는 길이가 1, model에게 2장을 주면 results는 길이가 2
        for r in results:
            # 프레임 1장에 있는 객체 개수 만큼 bbox가 있을 것이다.
            boxes = r.boxes 
            for box in boxes: # bbox 하나하나 정성들여서 이미지에 drawing 할거다.
                # bounding box
                # bounding box에 bbox의 좌효가 들어있다.
        ...
        return cv2.imencode('.jpg', image)[1].tobytes()
                # imencode -> 이미지 인코딩
                # '.jpg' 형식으로 만들고
                # image 즉, 텍스트와 바운딩 박스가 draw된 상태로 jpg 인코딩으로 반환함.ㅣ